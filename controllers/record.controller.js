...
module.exports = {
  ...
  post: async (req, res) => {
    ...
    students = await StudentModel.find({ listClass: req.body.classId })
      .lean()
      .select('_id')
  ...
  put: async (req, res) => {
    ...
    try {
      ...
      if (req.body.classId) {
        record.classId = req.body.classId
      }
      if (req.body.note) {
  ...
  getDetail: async (req, res) => {
    ...
    try {
      queriedRecord = await StudentRecordModel.find({ recordId: req.params.recordId })
        .lean()
        .populate({
          path: 'studentId',
          populate: {
  ...
  putStudentRecord: async (req, res) => {
    ...
    try {
      if (req.body.score) {
        studentRecord.score = req.body.score
      }
      if (req.body.note) {
        studentRecord.note = req.body.note
      }
