  ...
  get: async (req, res) => {
    ...
    if (matchedClass.length >= 2) {
      ...
      predictedClass = matchedClass.find((item) => {
        ...
        return (
          currentTimeMs >= startTimeMs - 30 * 60000 &&
          currentTimeMs <= startTimeMs + 30 * 60000
        )
      })
      ...
      return res.json({
        student: student,
        message: 'More Than 1 Class Found',
        class: matchedClass,
      })
    }
    if (matchedClass.length == 0) {
      ...
      const thisWeek = new Date(
        Date.now() - (currentWeekday - 1) * 24 * 60 * 60 * 1000 - timeToMs(currentTime),
      )
      ...
      var predictedMakeUpClass = availableClasses.find((item) => {
        return (
          item.subjectName == priorAbsencedClass[0].subjectName &&
          timeToMs(item.timetable.startTime) - timeToMs(item.timetable.startTime) ==
            timeToMs(priorAbsencedClass[0].timetable.startTime) -
              timeToMs(priorAbsencedClass[0].timetable.startTime)
        )
      })
      student.listClass = undefined
      if (predictedMakeUpClass) {
        predictedMakeUpClass.__v = undefined
      }
      return res.json({
        student: student,
        predictedClass: predictedMakeUpClass,
