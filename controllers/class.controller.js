...
module.exports = {
  ...
  get: async (req, res) => {
    ...
    let classQueried
    try {
      classQueried = await ClassModel.find(query)
        .lean()
        .populate({
  ...
  put: async (req, res) => {
    ...
    try {
      ...
      if (req.body.teacher) {
        matchedClass.teacher = req.body.teacher
      }
      if (req.body.grade) {
        matchedClass.grade = req.body.grade
      }
      if (req.body.mentor) {
        matchedClass.mentor = req.body.mentor
      }
  ...
  classView: async (req, res) => {
    ...
    // CLASS QUERY
    try {
      classView = await ClassModel.findById(req.params.id)
        .lean()
        .populate({
          path: 'mentorId teacherId',
          populate: {
  ...
  queryGrade: async (req, res) => {
    ...
    let classQueried
    try {
      classQueried = await ClassModel.find({ grade: grade })
        .lean()
        .populate({
          path: 'mentorId teacherId',
          populate: {
